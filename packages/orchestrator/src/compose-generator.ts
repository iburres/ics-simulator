...
const DEVICE_IMAGES: Record<DeviceCategory, string> = {
  ...
  // ISA-88 batch controller — same OpenPLC runtime as plc/safety-plc, real
  // S88 phase-sequencer ST generated by buildBatchProgram (plc-program-gen.ts).
  'batch-controller': 'ghcr.io/iburres/otforge-openplc:latest',
  // Real PMU — polls a wired generator process-unit over Modbus TCP and
  // re-exposes it as a real IEEE C37.118 synchrophasor stream (containers/pmu).
...
const DEVICE_LIMITS: Record<DeviceCategory, { memory: number; cpus: string }> = {
  ...
  'batch-controller': { memory: 128, cpus: '0.5' }, // ISA-88 batch controller — same OpenPLC budget
  pmu: { memory: 64, cpus: '0.25' }, // hand-rolled C37.118 server + Modbus poll loop, no ST runtime
...
export function generateCompose(
  ...
  // ── PMU → generator process-unit IP map ─────────────────────────────────────
  // Scan canvas edges to find which generator process-unit (if any) each PMU
  // is directly wired to — used below to inject GENERATOR_IP so the PMU's
  // Modbus poll loop can read real freq/voltage/power/reactive at container
  // startup (see containers/pmu/server.py). Direct-edge-only, no coilSource
  // indirection — a PMU is a monitoring tap on a generator bus, not part of
  // an actuator coil chain. Only process-units with processType 'generator'
  // are valid targets; a water-tank/pipeline/batch-reactor has no
  // frequency/voltage concept for a synchrophasor to report.
  const pmuToGeneratorNodeId = new Map<string, string>()
  for (const edge of scenario.visual.edges) {
    const srcDevice = scenario.devices.devices[edge.source]
    const tgtDevice = scenario.devices.devices[edge.target]
    if (!srcDevice || !tgtDevice) continue
    const isGenerator = (d: typeof srcDevice): boolean =>
      d.category === 'process-unit' && d.processUnit?.processType === 'generator'
    if (srcDevice.category === 'pmu' && isGenerator(tgtDevice)) {
      pmuToGeneratorNodeId.set(edge.source, edge.target)
    } else if (tgtDevice.category === 'pmu' && isGenerator(srcDevice)) {
      pmuToGeneratorNodeId.set(edge.target, edge.source)
    }
  }
  ...
  for (const [nodeId, device] of Object.entries(scenario.devices.devices)) {
    ...
    // plc, safety-plc, and batch-controller all run the OpenPLC runtime — same
    // port layout, same PROCESS_SIM_IP wiring, same IDE access. safety-plc and
    // batch-controller additionally receive their own category-specific env
    // vars for display in logs and the properties panel.
    if (
      device.category === 'plc' ||
...
`
      ...
      // Batch-specific env vars — injected for batch-controller devices only.
      // Informational (OpenPLC logs them at startup); the actual recipe
      // numbers are baked into the generated ST itself by buildBatchProgram,
      // not read from these env vars at runtime — same split as SIS above,
      // where SIS_VOTING is informational but votingConfig also drives
      // buildSafetyVotingProgram directly from the scenario object.
      if (device.category === 'batch-controller' && device.batch) {
        const batchEnv: string[] = services[serviceName].environment ?? []
        if (device.batch.recipeName) batchEnv.push(`BATCH_RECIPE_NAME=${device.batch.recipeName}`)
        if (device.batch.chargeTargetPct !== undefined)
          batchEnv.push(`BATCH_CHARGE_TARGET_PCT=${device.batch.chargeTargetPct}`)
        if (device.batch.heatSetpointC !== undefined)
          batchEnv.push(`BATCH_HEAT_SETPOINT_C=${device.batch.heatSetpointC}`)
        if (device.batch.reactHoldSec !== undefined)
          batchEnv.push(`BATCH_REACT_HOLD_SEC=${device.batch.reactHoldSec}`)
        if (device.batch.coolSetpointC !== undefined)
          batchEnv.push(`BATCH_COOL_SETPOINT_C=${device.batch.coolSetpointC}`)
        if (device.batch.dischargeTargetPct !== undefined)
          batchEnv.push(`BATCH_DISCHARGE_TARGET_PCT=${device.batch.dischargeTargetPct}`)
        services[serviceName].environment = batchEnv
      }
    }

    // PMU — real IEEE C37.118 synchrophasor device (containers/pmu). Not part
    // of the OpenPLC family above (no ST runtime, no ports published — reached
    // over the Docker network like ethernetip-adapter/profinet-device, not
    // the host). Injects GENERATOR_IP when wired to a generator process-unit;
    // PMU_* station-identity vars are injected unconditionally below in
    // buildDeviceEnv since they need no IP resolution.
    if (device.category === 'pmu') {
      const generatorNodeId = pmuToGeneratorNodeId.get(nodeId)
      if (generatorNodeId) {
        const generatorIp =
          claimedDeviceIps.get(generatorNodeId) ??
          resolveDeviceIp(
            scenario.devices.devices[generatorNodeId].ipAddress,
            scenario,
            effectiveZones
          )
        const pmuEnv: string[] = services[serviceName].environment ?? []
        pmuEnv.push(`GENERATOR_IP=${generatorIp}`)
        services[serviceName].environment = pmuEnv
      }
    }
...
function buildDeviceEnv(
  ...
  if (device.processUnit) {
    ...
    if (pu.heaterRateCPerSec !== undefined) env.push(`HEATER_RATE_C_PER_S=${pu.heaterRateCPerSec}`)
    if (pu.coolingRateCPerSec !== undefined)
      env.push(`COOLING_RATE_C_PER_S=${pu.coolingRateCPerSec}`)
  }
  ...
  // (shown in logs/properties panel); GENERATOR_IP is injected separately in
  // the outer per-device loop above, where claimedDeviceIps/resolveDeviceIp
  // are in scope, same split as PROCESS_SIM_IP vs. SIS_FUNCTION/SIS_VOTING.
  if (device.pmu) {
    if (device.pmu.idCode !== undefined) env.push(`PMU_IDCODE=${device.pmu.idCode}`)
    if (device.pmu.stationName) env.push(`PMU_STATION_NAME=${device.pmu.stationName}`)
    if (device.pmu.dataRateFps !== undefined)
      env.push(`PMU_DATA_RATE_FPS=${device.pmu.dataRateFps}`)
    if (device.pmu.nominalFreqHz !== undefined)
      env.push(`PMU_NOMINAL_FREQ_HZ=${device.pmu.nominalFreqHz}`)
  }
  ...
  // SIS_MBCONFIG_B64 injection below). batch-controller devices wired to a
  // batch-reactor process-unit get the real ISA-88 phase-sequencer instead —
  // see buildBatchProgram. Its PROCESS_SIM_IP/mbconfig.cfg wiring is the
  // existing single-process-unit path (no new IP-resolution code needed here).
  if (
    device.category === 'plc' ||
    device.category === 'safety-plc' ||
    device.category === 'batch-controller'
    ...
    const batchReactorNodeId =
      device.category === 'batch-controller'
        ? inferBatchReactorNodeId(scenario, device.nodeId)
        : undefined
    const program = device.plcProgram?.source
      ...
      : sisSensorNodeIds.length > 0
        ...
        : batchReactorNodeId
          ? buildBatchProgram(scenario, batchReactorNodeId, device.batch)
